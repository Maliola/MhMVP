...
public abstract class BaseWxShareHandler extends BaseShareHandler {
    ...
    public BaseWxShareHandler(Activity context, LibShareConfiguration configuration) {
        ...
        doOnMainThread(new Runnable() {
            ...
            public void run() {
                ...
                if (!result && getShareListener() != null) {
                    getShareListener().onError(getShareMedia(), ShareStatusCode.ST_CODE_SHARE_ERROR_SHARE_FAILED, new ShareException("sendReq failed"));
                }
            }
        });
    }
    ...
    private void parseResult(BaseResp resp, SocializeListeners.ShareListener listener) {
        switch (resp.errCode) {
            case BaseResp.ErrCode.ERR_OK:
                listener.onSuccess(getSocializeType(), ShareStatusCode.ST_CODE_SUCCESSED);
                break;
            ...
            case BaseResp.ErrCode.ERR_SENT_FAILED:
                listener.onError(getSocializeType(), ShareStatusCode.ST_CODE_SHARE_ERROR_SHARE_FAILED, new ShareException(resp.errStr));
                break;
        }
    }
