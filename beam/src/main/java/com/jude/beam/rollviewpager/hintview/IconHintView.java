package com.jude.beam.rollviewpager.hintview;
...
import com.jude.beam.rollviewpager.Util;
...
public class IconHintView extends ShapeHintView {
    ...
    public IconHintView(Context context,@DrawableRes int focusResId,@DrawableRes int normalResId,int size) {
        this(context, focusResId, normalResId, Util.dip2px(context,size),Util.dip2px(context,32));
    }

    public IconHintView(Context context, @DrawableRes  int focusResId, @DrawableRes int normalResId,int width,int height) {
        super(context);
    ...
    public Drawable makeFocusDrawable() {
        ...
        if (size>0){
            drawable = zoomDrawable(drawable,size,height);
        }
        return drawable;
    }
